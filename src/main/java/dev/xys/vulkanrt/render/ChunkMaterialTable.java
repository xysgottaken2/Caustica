...
public final class ChunkMaterialTable implements AutoCloseable {
    ...
    public ChunkMaterialTable(VulkanRayTracingContext context, CommandBatch batch, List<Entry> entries) {
        ...
        try {
            for (int i = 0; i < count; i++) pack(bytes, i * ROW_BYTES, entries.get(i));
    ...
    public static void pack(ByteBuffer bytes, int offset, Entry entry) {
        ...
        bytes.putInt(offset+16,color).putInt(offset+20,layout.vertexCount()).putInt(offset+24,layout.triangles()).putInt(offset+28,0);
