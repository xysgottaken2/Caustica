 ...
 * copy of the world color, and copy the result back to the world target at the end-of-world seam.
 * With RR disabled the trace runs at 1:1 and a linear blit stands in for the upscale.
 */
public final class RtComposite {
    ...
    // WorldPushData and its serializer are generated from Slang's reflected Std430DataLayout.
    ...
    // Sign of the sub-pixel jitter as reported to DLSS-RR + applied to the primary ray.
    ...
    // Set at the HEAD of Minecraft.reloadResourcePacks(): a reload recreates the block atlas + entity
    // textures, so the world pipeline is torn down and rebuilt once the NEW atlas handle appears.
    ...
    // A new material epoch makes the first composite return to vanilla so RtTerrain can full-clear.
    private boolean materialEpochTraceGate;
    private static final int PUSH_RING = 6;
    private PushSlot[] pushRing;
    private int pushSlot;
    private RtDisplayPipeline displayPipeline;
    // Mip-chain bloom feeding the display pass (prefilter -> downsample* -> upsample*). Sized alongside
    // the display images; its resolved half-res result is bound into the display pipeline.
    private RtBloomPipeline bloomPipeline;
    private RtImage output;
    // Packed primary -> indirect continuations (two records per render pixel).
    private RtBuffer continuationQueue;
    private RtImage displayImage;
    // Parallel PQ-encoded ([0,1], ST.2084) HDR display image, written alongside displayImage in HDR mode.
    private RtImage hdrDisplayImage;
    private boolean hdrWrittenThisFrame;
    // DLSS-FG "hudless" resource: a copy of the main render target before the UI overlay composites back.
    private RtImage fgHudlessImage;
    // HDR counterpart of fgHudlessImage, copied in presentHdr before its own UI composite runs.
    private RtImage fgHdrHudlessImage;
    // Composites the combined UI overlay over hdrDisplayImage at paper white, just before present.
    ...
    // Menu/non-RT present: converts the SDR main target (sRGB) to PQ-encoded at paper white.
    private RtSdrPresentPipeline sdrPresentPipeline;
    private RtImage sdrPresentImage;
    // DLSS Frame Generation: per-generated-frame interpolated output images + reprojection matrices.
    ...
    // Guide buffers (first-hit attributes for DLSS-RR).
    ...
    // Display-res RT image the display mapper reads (DLSS-RR target, or the blit-upscale fallback).
    ...
    // Camera captured each frame from GameRenderer.
    ...
    // Per-frame TLAS resources, rebuilt in place from a small ring of persistent slots.
    private final RtAccel.TlasRing tlasRing = new RtAccel.TlasRing();

    // This frame's TLAS handle, published after prepareTlas for the world-overlay occlusion pass.
    ...
    /** This frame's TLAS handle (0 if none built yet), for the overlay package's occlusion queries. */
     ...
     * A deliberately transient {@link #composite} return is not a renderer failure and must not trip
     * {@code VanillaRenderController}'s permanent safety latch.
     */
    ...
    /** Clear the failure latch on an explicit render-state invalidation (F3+A, dimension change). */
     ...
     * traced with. Valid after {@code updateMotion} ran this frame; do not mutate.
     */
    ...
    /**
     * Reset per-frame present state at the very start of GameRenderer render, so menu/no-world frames do
     * not present a stale HDR image behind the menu.
     */
    ...
    public boolean composite(GpuTexture nativeColor, int width, int height) {
        ...
        // Count-bounded terrain streaming runs here once per render frame — before the ready gate below,
        // because it is what MAKES terrain ready during the initial fill.
        try {
            RtTerrain.frame(ctx);
        } catch (Throwable t) {
            ctx.gpuExecutor().throwIfFailed();
        ...
        if (RtTerrain.currentOrNull() == null || !frameCaptured || Minecraft.getInstance().level == null) {
            // No world this frame: skip RT so the present path falls back to vanilla SDR / PQ SDR convert.
            return false;
        }
        try {
            ...
            // Skip RT until the reloaded block atlas handle is a fresh non-zero value we have not bound.
            if (reloadRebindRequested) {
                long atlas = blockAlbedoAtlasView();
            ...
            // Idempotent every frame: a manual -> auto exposure switch needs its resources before record().
     ...
     * loaded, before terrain tessellates, so the immutable material snapshot is available to the first
     * worker section. Driven from the client tick ahead of {@link RtTerrain#update}.
     */
     ...
     * and the canonical material page bundles in reserved bindless slots.
     */
    private void bindWorldTextures(RtContext ctx) {
        ...
        // Bind the vanilla celestials atlas (sun + moon phases) for world.rmiss.
    ...
    /** Vulkan image-view of the vanilla celestials atlas (sun + moon-phase sprites), or 0. */
    ...
    /**
     * Hooked at the HEAD of {@code Minecraft.reloadResourcePacks()}. Drains in-flight frames and destroys
     * the world pipeline so MC can free the old atlas textures cleanly; {@code ensureWorld} rebuilds it
     * on the first world frame after the reload.
     */
    ...
    private void ensureOutput(RtContext ctx, int width, int height) {
        ...
        displayW = width;
        displayH = height;
        // With RR on, ask NGX what render resolution its chosen quality mode actually expects.
        ...
        // RT traces into an HDR target so radiance > 1 survives to the display mapping seam. displayImage
        // stays R8G8B8A8 to match the main target it is copied into.
    ...
    /**
     * Compute this frame's motion-vector push data: the matrix that projects a current world point into
     * the previous frame's clip space, plus the per-frame camera translation.
     */
    ...
    private void recordFrame(RtContext ctx, RtPipeline active, GpuTexture nativeColor) {
        ...
        try (MemoryStack stack = MemoryStack.stackPush(); RtDebugLabels.Scope frameLabel = RtDebugLabels.scope(ctx, cmd, "composite frame")) {
            ...
            // flags bit 0: camera-in-water. bit 4: W1 animated wave normals.
            int flags = 0;
            var level = Minecraft.getInstance().level;
            if (level != null) {
                cameraBlockPos.set(Mth.floor(camX), Mth.floor(camY), Mth.floor(camZ));
                // Height-aware, mirroring vanilla's own Camera.getFluidInCamera().
            ...
            // Water parameters: camera-biome tint plus wrapped animation time.
            float wtr = 0.25f, wtg = 0.46f, wtb = 0.9f;
            if (level != null) {
            ...
            // Wave-domain anchor: the terrain rebase origin reduced mod 4096.
            Float4 waterAnchor = new Float4(terrain.blockX & WATER_ANCHOR_MASK,
                    terrain.blockZ & WATER_ANCHOR_MASK, priorWaterWaveTime, 0f);

            // Rebuild the TLAS this frame from static section instances merged with dynamic entity ones.
            ...
            new WorldPushData(
                    ...
                    // slider dims or lifts the whole night, not just the moon's direct light.
            ...
            // When RR did not produce the display-res image, bring the render-res trace up with a blit.
            if (!rrDone) {
            ...
            // Auto-exposure meters rrOutput (the post-RR, denoised image), not the noisy pre-RR trace, so
            // the histogram's log-luminance average does not drift with SPP.
            ...
            // Bloom runs between exposure and the display mapping: the prefilter thresholds in the same
            // exposed domain the tonemap curve works in, and the display pass needs the finished chain.
        ...
        encoder.execute(cmd); // deferred into the frame's submission
        RtEntities.INSTANCE.markGraphicsUse(frameEntities, graphicsUse);
    }

    /**
     * Block-breaking overlay: mirrors vanilla's {@code ClientLevel.destructionProgress()} into the push's
     * {@code breaking[]} list, so {@code world.rchit} can blend the destroy-stage crack texture into a hit
     * block's albedo. Those textures ride the same bindless array as entity textures.
     */
     ...
     * Celestial angles come from the camera's attribute probe (partial-tick interpolated).
     */
    private SkyPush skyPush() {
        ...
        if (sunY > -0.05f) {
            ...
            // never disagree with the sky behind it. The old hand-authored tint ladder is gone; all that
            // survives is a small warm residual, scaled by SUNSET_TINT_STRENGTH.
            float sunPeak = 21.0f;
            ...
            // dozens of air masses and most of the scene is lit by skylight instead.
    ...
    /**
     * Push the celestials-atlas UV rects for the sun sprite and the current moon-phase sprite, so
     * world.rmiss can sample the real vanilla textures on the discs.
     */
    ...
    private void refreshCelestialUvCache(int moonPhase) {
        ...
        try {
            ...
            // celestials atlas not yet loaded — keep full-range UVs
        }
     ...
     * the NEE sun/moonlight, so terrain light and the sky's sunset can never disagree.
     */
    ...
    /** Whether the HDR present path should replace the vanilla SDR blit. */
    ...
    /** DLSS-FG: the PQ-encoded HDR backbuffer view, valid right after {@link #presentHdr} ran. */
     ...
     * blit. The combined UI image is blended over the HDR image here at paper white before the blit; the
     * magic stage/access values mirror vanilla {@code blitFromTexture} exactly, Y-flip included.
     */
    public void presentHdr(VulkanCommandEncoder enc, long swapchainImage, int swapW, int swapH, long acquireSem, long presentSem) {
        ...
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            // DLSS-FG "hudless" capture: snapshot the pre-UI world image before the composite below.
            if (RtDlssFg.enabled()) {
            ...
            // Swapchain UNDEFINED -> TRANSFER_DST, plus make the HDR compute writes visible to the blit.
    ...
    /** Lazily create the HDR UI-composite compute pipeline + its nearest/clamp sampler. */
     ...
     * present instead of vanilla's raw SDR blit.
     */
    ...
    /**
     * Present a non-RT (menu/loading) frame to the PQ swapchain: convert the SDR main target to
     * PQ-encoded at paper white via a compute pass, then blit that into the swapchain. Mirrors
     * {@link #presentHdr} barrier-for-barrier; returns false (keep vanilla SDR blit) if unavailable.
     */
     ...
     * non-RR / fallback upscale; a no-op stretch when the two are the same size.
     */
    ...
    /**
     * DLSS Frame Generation quality: capture a copy of {@code main} into {@link #fgHudlessImage} for
     * {@link #fgInterpolate} to feed DLSSG as the "hudless" resource. Called from GameRendererMixin right
     * after GuiRenderer.render() but BEFORE the UI overlay composite. No-op unless both FG and the UI
     * overlay redirect are active.
     */
    public void captureFgHudless(RenderTarget main) {
        ...
        try {
            ...
            return; // not a Vulkan-backed texture
        }
    ...
    /**
     * HDR counterpart of {@link #captureFgHudless} — copies this frame's pre-UI {@code hdrDisplayImage}
     * into {@link #fgHdrHudlessImage}. A plain copy, not a conversion: both images are already PQ-encoded.
     */
     ...
     * into Minecraft's command encoder, returning the interpolated output image for the presenter to blit.
     * Returns null when there is no captured RT frame to interpolate from (menu/loading frames); throws
     * for real failures so the caller can disable FG for the session. Rotation-only matrices; camera
     * translation is carried by the mvecs. {@code hdrBackbuffer} selects the PQ path.
     */
    public RtImage fgInterpolate(VulkanCommandEncoder enc, long backbufferView, long backbufferImage,
        ...
        if (index == 1) {
            ...
            // clipToPrevClip = prevVP * inverse(curVP); prevClipToClip = curVP * inverse(prevVP).
